const globalLanguagesRawDataset = `
639-1 ,639-2/T ,639-2/B ,LanguageName ,NativeName 
...
tl ,tgl ,tgl ,Tagalog ,Wikang Tagalog
...
zu ,zul ,zul ,Zulu ,isiZulu `.trim();

const globalLanguagesDataset = [];

function loadGlobalLanguagesDataset() {
  const lines = globalLanguagesRawDataset.split('\n');
  const columnNames = lines[0].split(',').map(x => x.trim());
  
  for (let i = 1;i < lines.length;i++) {
    const row = lines[i];
    const values = row.match(/(".*?"|[^",\s]+)(?=\s*,|\s*$)/g).map(x => x.replaceAll('"', '').trim());
    let obj = {};

    for (let j = 0; j < columnNames.length;j++) {
      obj[columnNames[j]] = values[j];
    }

    globalLanguagesDataset.push(obj);
  }
}

loadGlobalLanguagesDataset();

function getLanguageInfo(colName, colValue) {
  for (let i = 0; i < globalLanguagesDataset.length;i++) {
    obj = dataset[i];

    if (obj[colName] === colValue) {
      return obj;
    }
  }
}
